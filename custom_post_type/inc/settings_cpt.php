...
class BcCustomPostTypeOptions {
	...
	public function bc_custom_post_type_create_admin_page() {
		...
		<div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background-color: #000; opacity: 0.8; z-index: 99999;" id="pre_bg">
			<img src="<?php echo plugin_dir_url( DIR_COMPONENT .  '/bweb_component_functions/' )."custom_post_type/assets/loading.gif"?>" style="width: 32px; position: fixed; top: 50%; left: 50%; transform: translate(-50%, 50%);" />
		</div>
    ...
    public function load_enqueue(){
		...
		wp_enqueue_style( 'settings_cpt-css', plugin_dir_url( DIR_COMPONENT .  '/bweb_component_functions/' ).'custom_post_type/assets/style.css');
    }
